...
const BannerSlide = ({ singleData }) => {
	const { id, title, subTitle, desc, img, btnText, link } = singleData;
	return (
		<div
			style={{
		...
		>
			<div className="text-start mb-10 px-5 ">
				...
				<p className="bg-darkBg bg-opacity-50 px-3 py-7 rounded-md max-w-4xl text-sm lg:text-base">
					{desc}
				</p>

				<Link href={link}>
					<button className="bg-green-500 hover:bg-green-700 rounded-sm text-white px-5 lg:px-10 py-2 lg:py-3 text-bold text-xl lg:text-2xl mt-10">
						{btnText}
					</button>
				</Link>
			</div>
